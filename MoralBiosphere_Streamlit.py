...
def main():
    ...
    with st.sidebar:
        ...
        negative_adjustment_rate = st.slider("Negative Adjustment Rate", -0.01, -0.001, -0.005, 0.001,
                                             format="%.3f",
                                             help="Strength of divergence from distrusted critiques (negative value).")
        critique_threshold = st.slider("Critique Threshold", 0.05, 0.5, 0.05, 0.05,
                                       help="Minimum moral difference to trigger a critique.")
        ais_strength = st.slider("AIS Strength", 0.1, 1.0, 0.5, 0.1,
                                 help="Amplification factor for AIS probes.")
        num_ais_systems = st.slider("Number of AIS Systems", 1, 20, 12,
